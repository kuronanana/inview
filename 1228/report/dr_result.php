...
<body>
  ...
  session_start();
  $dr_result = $_POST['date'];
  $timestamp = strtotime($dr_result); //將字串轉成時間戳記
  $date = date('Y-m-d', $timestamp); //將收到的時間戳記以日期格式丟給$dat
  ...
  <div class="con">
    ...
    while ($row = mysqli_fetch_array($result)) { //每找到一筆資料就做一次迴圈內的程式
      ...
      $key_date = date('Y-m-d', $key_timestamp); //將收到的時間戳記以日期格式丟給$key_date
      if ($date == $key_date) { //日期符合
        $price = $row[5] * $row[6];
        $total = $total + $price;
        echo "<tr>";
