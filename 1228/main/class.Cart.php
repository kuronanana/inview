...
class Cart
{
	...
	{
		// $quantity = (preg_match('/^\d+$/', $quantity)) ? $quantity : 1; //判斷輸入是否數字
		// $attributes = (is_array($attributes)) ? array_filter($attributes) : [$attributes];
		$hash = md5(json_encode($attributes)); //json_encode轉成字串 //md5 加密

		// if (count($this->items) >= $this->cartMaxItem && $this->cartMaxItem != 0) {
		// 	return false;
		// }
		//this
		if (isset($this->items[$id])) {    //id=店名
